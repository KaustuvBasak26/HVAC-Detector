...
- **512 MB RAM** — OpenCV/PDF work is memory-heavy. The Render blueprint sets `HVAC_LOW_MEMORY_MODE=true`, caps rasterization at **96 DPI** and **2400 px** on the longest side, and skips heavy measure-label drawing so `testset2.pdf` fits; very large sheets may still OOM on Free. Upgrade to Starter if needed. The bundled **`Sample annotation.png`** calibrated mask is still used for the sample plan (detection quality does not fall back to auto-detect on Render).
